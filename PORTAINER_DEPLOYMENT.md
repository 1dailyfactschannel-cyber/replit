...
ALLOWED_ORIGINS=http://10.30.0.136:8090

# URL приложения
APP_URL=http://10.30.0.136:8090

# Разрешить HTTP (обязательно для работы без HTTPS)
ALLOW_HTTP=true
...
4. Убедитесь, что `ALLOW_HTTP=true` установлен (иначе авторизация не будет работать без HTTPS)

### Ошибка авторизации / сессии не сохраняется
- Проверьте, что `ALLOW_HTTP=true` установлен в переменных окружения
- Без этого флага cookies сессии устанавливаются с флагом `secure`, что блокирует их при HTTP-соединении

### Ошибка подключения к базе данных: `CONNECT_TIMEOUT 10.30.0.136:5532`
Это означает, что контейнер не может достучаться до PostgreSQL. Диагностика:

1. **Проверьте доступность с Docker-хоста:**
   ```bash
   ping 10.30.0.136
   nc -zv 10.30.0.136 5532
   ```

2. **Проверьте доступность из контейнера:**
   ```bash
   docker exec teamsync-server ping -c 3 10.30.0.136
   ```

3. **Если пинг проходит, но порт недоступен** — PostgreSQL слушает только `localhost`. На сервере БД исправьте:
   - `postgresql.conf`: `listen_addresses = '*'`
   - `pg_hba.conf`: добавьте строку `host all all 0.0.0.0/0 md5` (или конкретную подсеть Docker)

4. **Если пинг не проходит** — сеть недоступна. Варианты:
   - Если БД на том же физическом хосте, что и Docker — замените в `.env`:
     ```
     DATABASE_URL=postgresql://db_test:D2rGkB6CaIwpb@host.docker.internal:5532/db_test
     ```
   - Если БД в другой сети — настройте маршрутизацию или VPN между сетями
   - В крайнем случае для теста можно использовать `network_mode: host` в `docker-compose.yml` (раскомментируйте строку)
